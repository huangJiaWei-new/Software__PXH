...
cv::Mat FindRectCenterDlg::m_binImg     = cv::Mat::zeros(3, 3, CV_8UC1);
...
static void addExclusionArea(int event,int x,int y,int flags,void *userdata)
{
    ...
    if(m_binImg->channels() == 1) //保证能显示其它颜色
    {
        cv::cvtColor(*m_binImg, *m_binImg, cv::COLOR_GRAY2BGR);
    }
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        {         
            ...
            //绘制矩形，红色
            cv::rectangle(*m_binImg, startPoint_rect, endPoint_rect, cv::Scalar(0, 0, 255), 2);
...
//    }
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        else if (selectAreaCount == 1)
        {
            ...
            //绘制矩形
            cv::rectangle(*m_binImg, startPoint_rect, endPoint_rect, cv::Scalar(0, 255, 0), 2);
...
{  
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        else if (selectAreaCount == 1)
        {
            ...
            qDebug() << "sobelImg" << sobelImg.channels();
            ...
            //cv::cvtColor(sobelImg, gray_SobelImg, cv::COLOR_BGR2GRAY); sobelImg已经是单通道图像

            //寻找交点
            intersectionPoint_Line = FindRectCenterDlg::findIntersections(sobelImg, lineImage, box);
            // ------------↑

            //保证绘制的直线与图像边缘有交点，然后存入左边缘线容器、包含所有直线的容器
            if (intersectionPoint_Line.x != 0 && intersectionPoint_Line.y != 0)
            {
                cv::line(*m_binImg, startPoint_line, endPoint_line, cv::Scalar(255, 0, 0), 1);
                cv::imshow("test", *m_binImg);
...
{   
    ...
    static std::vector<BindLineAndRect> vector_leftLine_leftSelectArea; //
    std::vector<BindLineAndRect> commonElements;
    ...
    vector_leftLine_leftSelectArea = FindRectCenterDlg::get_leftLineAddleftSelectAreaVec();
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        vector_ProcessedLine = FindRectCenterDlg::updataAllLineInfo(minLineDistanceIndex,
                                                                    ...
                                                                    vector_DownLine);
        // 定义两个容器来分别存放直线和矩形
        /*std::vector<LineInfo> vectorLines;
        std::vector<RectangleInfo> vectorRectangles;

        //将直线与矩形取出，更新'vector_leftLine_leftSelectArea'容器
        for (const auto& bindLineAndRect : vector_leftLine_leftSelectArea)
        {
            // 将直线存放到 vectorLines
            vectorLines.push_back(bindLineAndRect.lineInfo);
            // 将矩形存放到 vectorRectangles
            vectorRectangles.push_back(bindLineAndRect.rectangleInfo);
        }*/

        /*for (const BindLineAndRect& item : vector_leftLine_leftSelectArea)
        {
            // 检查是否存在相同的矩形
            auto rectIter = std::find_if(vector_ProcessedRect.begin(), vector_ProcessedRect.end(),
                [&item](const RectangleInfo& rect) {
                    return item.rectangleInfo == rect; //假设RectangleInfo具有适当的相等运算符
                });

            // 检查是否存在相同的直线
            auto lineIter = std::find_if(vector_ProcessedLine.begin(), vector_ProcessedLine.end(),
                [&item](const LineInfo& line) {
                    return item.lineInfo == line; //假设LineInfo具有适当的相等运算符
                });

            // 如果找到相同的矩形和直线，则将元素添加到commonElements中
            if (rectIter != vector_ProcessedRect.end() && lineIter != vector_ProcessedLine.end())
            {
        ...
        //将绘制的直线与选取区域绑定在一起
        //FindRectCenterDlg::setBindLineAndRect(vector_LeftLine, vector_rect.back());
        //bindLineAndRect = FindRectCenterDlg::getBindLineAndRect();
        //将绘制的直线与选取区域绑定在一起的集合放入容器，左边
        //FindRectCenterDlg::set_leftLineAddleftSelectAreaVec(bindLineAndRect);

        //vector_EclusionArea = FindRectCenterDlg::getEclusionAreaVec();
        //qDebug() << "删除后筛选区域的个数" << vector_EclusionArea.size();
                                   ...
                                   const QString &fileName)
{
    ...
    //存入绘制的左边缘直线
    for (const LineInfo &item : vector_LeftLine)
    {
         jsonArray_LeftLine.append(allLineToJson(item));
    }
    jsonObject["LeftLineVector"] = jsonArray_LeftLine;

    //存入绘制的右边缘直线
    for (const LineInfo &item : vector_RightLine)
    {
         jsonArray_RightLine.append(allLineToJson(item));
    }
    jsonObject["RightLineVector"] = jsonArray_RightLine;

    //存入绘制的上边缘直线
    for (const LineInfo &item : vector_UpLine)
    {
         jsonArray_UpLine.append(allLineToJson(item));
    }
    jsonObject["UpLineVector"] = jsonArray_UpLine;

    //存入绘制的下边缘直线
    for (const LineInfo &item : vector_DownLine)
    {
         jsonArray_DownLine.append(allLineToJson(item));
    }
    jsonObject["DownLineVector"] = jsonArray_DownLine;
...
/** --------------@brief：缩放图像------------- **/
/** --------------@note： ------------- **/
void FindRectCenterDlg::wheelEvent(QWheelEvent *event)
{
//    int w = m_imgOrigin.cols; //原图宽高
//    int h = m_imgOrigin.rows;
//    if(event->delta() > 0)//放大
//    {
//        double zoom = g_iZoom + g_iRadio;

//        if (w * zoom * h * zoom < 9000 * 9000)//图片宽高<3000*3000时缩放值才会变大
//        {
//            g_iZoom = zoom;
//        }
//        cv::resize(m_imgOrigin, matScaleOut, cv::Size(), g_iZoom, g_iZoom, cv::INTER_LINEAR); //按比例放
//        //cv::resizeWindow(winName,cv::Size(matOut.cols+50,matOut.rows+50));
//        cv::imshow(winName,matScaleOut);

//    }
//    else//缩小
//    {
//        double zoom = g_iZoom - g_iRadio;
//        if (w * zoom * h * zoom > 100 * 100)//图片宽高<3000*3000时缩放值才会变大
//        {
//            g_iZoom = zoom;
//        }
//        cv::resize(m_imgOrigin, matScaleOut, cv::Size(), g_iZoom, g_iZoom, cv::INTER_LINEAR); //按比例放
...
void FindRectCenterDlg::receiveUpImg(cv::Mat UpImg, int index_Img)
{
    ...
    if(m_UpImage.data == NULL)
    {
        qDebug() << "m_UpImage为空";
    }

    //判断图像显示窗口是否存在，如果存在则关闭重新创建显示窗口
    if(cv::getWindowProperty(winName, cv::WND_PROP_VISIBLE) == double(1.0))
    {
...
/** --------------@brief：收'cameradialog'界面传过来的俯视相机图像------------- **/
/** --------------@note： ------------- **/
void FindRectCenterDlg::receiveDownImg(cv::Mat DownImg, int index_Img)
{
    isUpImgorDownImg = index_Img; //0仰望，1俯视
    m_DownImage = DownImg;

    if(m_DownImage.data == NULL)
    {
        qDebug() << "m_DownImage为空";
    }
    ...
    ui->widgetPicture->setMinimumSize(m_DownImage.cols+60,m_DownImage.rows+60);
   // ui->widgetColor->setMinimumSize(imgOrigin.cols+60,imgOrigin.rows+60);
   // ui->widgetColor->hide();
    ui->scrollArea->widget()->setMinimumSize(m_DownImage.cols+60,m_DownImage.rows+60);

    cv::imshow(winName,m_DownImage);
    namedWindowToWidget(winName,ui->widgetPicture);
    //cv::destroyWindow(winName);
}
...
void FindRectCenterDlg::on_btnPreProcess_clicked()
{
    ...
    case 1:
        if(m_DownImage.data)
        {
            bool isSobelFlag = mySobel(m_DownImage);
            if(isSobelFlag)
            {
                QMessageBox::information(NULL, "PROMPT(提示)", "Sobel边缘检测成功");
            }
...
void FindRectCenterDlg::on_btnExcludeProcess_clicked()
{
    ...
    case 0:
        ...
        cv::setMouseCallback(winName, addExclusionArea, (void*)(&m_binImg));
...
void FindRectCenterDlg::on_btnJsonProcess_clicked()
{
    ...
    static std::vector<LineInfo> vec_LeftDrawLine;
    static std::vector<LineInfo> vec_RightDrawLine;
    static std::vector<LineInfo> vec_UpDrawLine;
    static std::vector<LineInfo> vec_DownDrawLine;
    ...
    case 0:
        ...
        ui->btnPreProcess->setEnabled(true); //使能'图像预处理'按钮
        ...
        vec_LeftDrawLine    = FindRectCenterDlg::getLeftLineVec(); //获取左、右、上、下边缘线容器
        vec_RightDrawLine   = FindRectCenterDlg::getRightLineVec();
        vec_UpDrawLine      = FindRectCenterDlg::getUpLineVec();
        vec_DownDrawLine    = FindRectCenterDlg::getDownLineVec();

        saveToJson(vector_eclusionArea, vec_selectArea,
                   vec_LeftDrawLine,    vec_RightDrawLine,
                   vec_UpDrawLine,      vec_DownDrawLine, finnal); //存入Json文件

        m_allInfoCollection.vec_SelectArea.clear(); // 清除向量中的所有元素
        m_allInfoCollection.vec_EclusionArea.clear();
        m_allInfoCollection.vec_AllRect.clear();
        m_allInfoCollection.vec_LeftLine.clear();
        m_allInfoCollection.vec_RightLine.clear();
        m_allInfoCollection.vec_UpLine.clear();
        m_allInfoCollection.vec_DownLine.clear();
        m_allInfoCollection.vec_AllDrawLine.clear();
        m_allInfoCollection.vec_FitLine.clear();
        m_allInfoCollection.vec_leftLine_leftSelectArea.clear();
        m_allInfoCollection.vec_rightLine_rightSelectArea.clear();
        m_allInfoCollection.vec_upLine_upSelectArea.clear();
        m_allInfoCollection.vec_downLine_downSelectArea.clear();
...
/** --------------@brief：------------- **/
/** --------------@note： ------------- **/
void FindRectCenterDlg::on_switch1_clicked()
{
    qDebug() << "容量1" <<  m_allInfoCollection.vec_SelectArea.size();
    qDebug() << "容量2" <<  m_allInfoCollection.vec_EclusionArea.size();
    qDebug() << "容量3" <<  m_allInfoCollection.vec_AllRect.size();
    qDebug() << "容量4" <<  m_allInfoCollection.vec_LeftLine.size();
    qDebug() << "容量4" <<  m_allInfoCollection.vec_RightLine.size();
    qDebug() << "容量5" <<  m_allInfoCollection.vec_UpLine.size();
    qDebug() << "容量6" <<  m_allInfoCollection.vec_DownLine.size();
    qDebug() << "容量7" <<  m_allInfoCollection.vec_AllDrawLine.size();
    qDebug() << "容量8" <<  m_allInfoCollection.vec_FitLine.size();
    qDebug() << "容量9" <<  m_allInfoCollection.vec_leftLine_leftSelectArea.size();
    qDebug() << "容量10" <<  m_allInfoCollection.vec_rightLine_rightSelectArea.size();
    qDebug() << "容量11" <<  m_allInfoCollection.vec_upLine_upSelectArea.size();
    qDebug() << "容量12" <<  m_allInfoCollection.vec_downLine_downSelectArea.size();
