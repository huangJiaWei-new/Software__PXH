...
static void drawLeftLine(int event,int x,int y,int flags,void *userdata)
{
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        else if (selectAreaCount == 1)
        {
            ...
            //qDebug() << "起点x的坐标" <<startPoint_line.x<< "Y的坐标" <<startPoint_line.y;
            //qDebug() << "终点x的坐标" <<endPoint_line.x<< "Y的坐标" <<endPoint_line.y;
...
{   
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        *m_binImg = image.clone();  // 或者使用 copyTo() 方法
        ...
        {
            RectangleInfo removedRect = vector_AllRect[minRectDistanceIndex];
            //删除矩形
            vector_AllRect.erase(vector_AllRect.begin() + minRectDistanceIndex);

            //查找在 vector_SelectArea 中与被删除的矩形相同的元素
            auto it = std::find_if(vector_SelectArea.begin(), vector_SelectArea.end(), [&](const RectangleInfo& line)
                                 {
                                   return compareRect(line, removedRect);
                                                                         });
            //如果找到了匹配的元素，则从 vector_SelectArea 中删除它
            if (it != vector_SelectArea.end())
            {
                vector_SelectArea.erase(it);
            }
            //打印更新后的 vector_SelectArea
            for (const auto& rect : vector_SelectArea)
            {
                qDebug() << rect.rectStartPoint.x;
                qDebug() << rect.rectEndPoint.x;
            }
            //更新数据源
            ...
            allInfoCollection.vec_SelectArea = vector_SelectArea; //更新选择区域容器
        ...
        /*if(minDistanceIndex != -1)
        {
            ...
            {
                vector_AllLine.erase(vector_AllLine.begin() + minDistanceIndex); //删除距离最短的直线

                //更新数据源，左边缘线容器
                AllInfoCollection &allInfoCollection = FindRectCenterDlg::getAllInfoCollection();
                allInfoCollection.vec_LeftLine = vector_AllLine;

                //清空图像并重新绘制剩余的直线
                image = cv::imread("D:/QT_Training/PXHForNick/binImg.jpg");

                //重新绘制直线
                for (const auto &line : vector_AllLine)
                {
                    cv::line(image, line.lineStartPoint, line.lineEndPoint, cv::Scalar(255, 0, 0), 1);
                }
...
/** --------------@brief：设置所有矩形的容器------------- **/
/** --------------@note： 静态成员函数------------- **/
void FindRectCenterDlg::setAllAreaVec(RectangleInfo allRcet)
{
...
/** --------------@brief：设置包含所有绘制直线的容器------------- **/
/** --------------@note： 静态成员函数------------- **/
void FindRectCenterDlg::setAllLineVec(LineInfo allLineVec)
{
